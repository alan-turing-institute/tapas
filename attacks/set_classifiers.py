...
from utils.data import encode_data, get_num_features
...
class SetReprClassifier(SetClassifier):
    ...
    """
    def __init__(self, SetRep, Classifier, meta_dict):
        self.SetRep = SetRep(meta_dict)
        self.Classifier = Classifier(self.SetRep.meta_dict_out)
...
class LRClassifier:
    ...
    def __init__(self, meta_dict):
        self.num_features = meta_dict['num_features']
...
class NaiveRep: # TODO: Write tests
    ...
    def __init__(self, meta_dict):
        self.meta_dict = meta_dict
        self.meta_dict_out = {'num_features': 3*get_num_features(self.meta_dict)}

    def get_representations(self, datasets):
        np_data = [encode_data(dataset, self.meta_dict) for dataset in datasets]
