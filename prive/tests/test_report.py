...
class MIAReport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        attacks = []
        for i in range(1000):
            attack_dict = {'labels': np.random.randint(2, size=100), 'predictions': np.random.randint(2, size=100),
                           'generator': np.random.choice(['RandomGenerator1', 'RandomGenerator2', 'RandomGenerator3']),
    ...
    def test_create_report(self):
        filepath_timestamp = os.path.join(os.path.dirname(__file__),
