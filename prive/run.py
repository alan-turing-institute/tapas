...
def main():
    ...
    parser.add_argument(
        "--genpath", "-G", type=str, help="Path to generator executable"
    )
    parser.add_argument(
        "--datapath",
        "-D",
    ...
    sdg_model = GeneratorFromExecutable(
        args.genpath
    )  # instance of generators.Generator
    ...
    threat_model = TargetedMIA(
        ...
        target,
        BlackBoxKnowledge(sdg_model, runconfig["synthetic_data_size"]),
    )
    ...
    for name, res in results.items():
        print(
            f'Accuracy ({name}): {np.mean(np.array(res["predictions"]) == np.array(res["test_labels"]))}'
        )
